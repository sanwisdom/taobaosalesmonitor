...
public class TmallProductItemDealsParser {
	...
	public TmallProductItemDealsParser(Date dealFrom, Date dealTo) {
		this.dealFrom = dealFrom;
		this.dealTo = dealTo;
	}
	
	static Logger log = Logger.getLogger(TmallProductItemDealsParser.class);
	...
	public DealInfo parse(Document dealsDoc) {
		...
		int size = monthRecordDetails.size() - 1;
		if (size <= 0) {
			return page;
		}
...
//		Date dateExcludeTo = parseDate("2012-11-11 23:59:59");
		for (Iterator<Element> iter = iterator; iterator.hasNext();) {
			...
			Element priceElement = element.select("tr > td > em").get(0);
